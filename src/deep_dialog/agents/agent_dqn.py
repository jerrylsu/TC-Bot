...
class AgentDQN(Agent):
    ...
    def initialize_episode(self):
        ...
        self.request_set1 = ['self_name', 'digits_key']  # 是公司员工
        self.request_set2 = ['is_reserve_visitor', 'self_name', 'phone_key']  # 不是公司员工，有预约
        self.request_set3 = ['is_reserve_visitor', 'host_name']  # 不是公司员工，没有预约


    def state_to_action(self, state):
        """ DQN: Input state, output action """
        if state.get('user_action') == None:
            # 没有获取到user的输入，这时将agent作为first表达者
            act_slot_response = {}
            act_slot_response['diaact'] = 'request'
            act_slot_response['request_slots'] = {'is_staff': 'UNK'}
            act_slot_response['inform_slots'] = {}
            # initialize, agent ask a question
            #act_slot_response['nl'] = '您好, 请问您是我公司员工吗?'
            #act_slot_response['turn'] = 0
            self.action = 0
        else:
            # 获取到user input
    ...
    def rule_policy(self, state):
        ...
        if state['current_slots']['inform_slots']['is_staff'] == u'True':
            if self.current_slot_id < len(self.request_set1):
                slot = self.request_set1[self.current_slot_id]
                self.current_slot_id += 1
                act_slot_response['request_slots'] = {slot: "UNK"}
            else:
                act_slot_response = {'diaact': "welcomestaff", 'inform_slots': {}, 'request_slots': {}}
            return self.action_index(act_slot_response)
        else:  # state['current_slots']['inform_slots']['is_staff'] == u'False':
            if self.current_slot_id == 0:   # 不是公司员工，立即问是否有预约
                act_slot_response['request_slots'] = {'is_reserve_visitor': "UNK"}
                self.current_slot_id += 1
                return self.action_index(act_slot_response)
            if state['current_slots']['inform_slots']['is_reserve_visitor'] == u'True':
                if self.current_slot_id < len(self.request_set2):
                    slot = self.request_set2[self.current_slot_id]
                    self.current_slot_id += 1
                    act_slot_response['request_slots'] = {slot: "UNK"}
                else:
                    act_slot_response = {'diaact': "welcomevisitor", 'inform_slots': {}, 'request_slots': {}}
                return self.action_index(act_slot_response)
            else:  # state['current_slots']['inform_slots']['is_reserve_vistor'] == u'False':
                if self.current_slot_id < len(self.request_set3):
                    slot = self.request_set3[self.current_slot_id]
                    self.current_slot_id += 1
                    act_slot_response['request_slots'] = {slot: "UNK"}
                else:
                    act_slot_response = {'diaact': "staffmeet", 'inform_slots': {}, 'request_slots': {}}
                return self.action_index(act_slot_response)
