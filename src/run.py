...
from deep_dialog.usersims import RuleSimulator, CustomRuleSimulator
...
if __name__ == "__main__":
    ...
    parser.add_argument('--act_set', dest='act_set', type=str, default='./deep_dialog/data/data_access/dia_acts_access.txt', help='path to dia act set; none for loading from labeled file')
    parser.add_argument('--slot_set', dest='slot_set', type=str, default='./deep_dialog/data/data_access/slot_set_access.txt', help='path to slot set; none for loading from labeled file')
...
all_goal_set = json.load(open(goal_file_path, 'rb'))
...
if usr == 0:# real user
    ...
    user_sim = CustomRuleSimulator(movie_dictionary, act_set, slot_set, goal_set, usersim_params)
...
def simulation_epoch(simulation_epoch_size):
    ...
    for episode in xrange(simulation_epoch_size):
        """ comment Jerry
            1. At each simulation epoch, we simulate N dialogues and add these state transition 
            tuples(s_t, a_t, r_t, s_t+1) to experience reply buffer pool.
            2. train and update the current DQN network. 
        """
...
def warm_start_simulation():
    """ comment by Jerry
        The warm up model is for initialize the experience replay buffer pool.
        A good initialization to speed up the RL trainning. Here we use a very simple rule-based policy.
    """
    successes = 0
    cumulative_reward = 0
    cumulative_turns = 0
...
def run_episodes(count, status):
    ...
    for episode in xrange(count):
        ...
        """ comment by Jerry
            while code block is a episode inference, a episode == multi-turns. not train code
            next_turn():
            RL model: agent.state_to_action --> self.run_policy --> DQN model
            Simulator model: self.user.next() Agenda(ruled)-based model
        """
        while(not episode_over):
            episode_over, reward = dialog_manager.next_turn()
            cumulative_reward += reward
        ...
        if agt == 9 and params['trained_model_path'] == None:
            ...
            simulation_res = simulation_epoch(simulation_epoch_size)  # simulation_epoch_size = 100
            ...
            if simulation_res['success_rate'] >= best_res['success_rate']:
                """ comment by Jerry
                    Experience reply buffer pool update strategy:
                    1. At the beginning, we will accumulate all the experience tuples from simulation.
                    2. Flush the experience reply buffer pool till the current RL agent reaches a success rate threshold (0.30).
                    3. Use the experience tuples from the current RL agent to re-fill the buffer.
                    
                    The initial performance of the DQN is not good to generate enough good experience replay tuples, thus
                    we do not flush the experience replay pool till the current RL agent can reach a certain success rate
                    which we think it is good, for example, the performance of a rule-based agent. So, here, at every 100
                    simulation epoches, we estimate the success rate of the current DQN agnet, if the current DQN agent 
                    is better enough( > 0.3), the experience replay buffer pool will be flushed and re-filled like below.
                """
                if simulation_res['success_rate'] >= success_rate_threshold:  # success_rate_threshold = 0.30
            ...
            """ comment Jerry
                1. The current DQN agnet will be updated multiple times in function simulation_epoch(simulation_epoch_size).
                2. Ate the end of the function simulation_epoch(simulation_epoch_size), the target network will be updated
                only once by the current DQN agnet network. 
            """
